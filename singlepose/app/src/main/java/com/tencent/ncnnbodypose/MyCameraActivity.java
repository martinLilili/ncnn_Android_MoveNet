...
import com.tencent.ncnnbodypose.camera2.Camera2Helper;
import com.tencent.ncnnbodypose.camera2.Camera2Listener;
import com.tencent.ncnnbodypose.util.ImageUtil;

import java.io.ByteArrayOutputStream;
import java.util.concurrent.ExecutorService;
import java.util.concurrent.Executors;

public class MyCameraActivity extends Activity implements ViewTreeObserver.OnGlobalLayoutListener, Camera2Listener {
    ...
    private void initView() {
        ...
        surfaceView.getHolder().addCallback(new SurfaceHolder.Callback() {
            ...
            public void surfaceCreated(SurfaceHolder surfaceHolder) {
                ncnnbodypose.setOutputWindow(surfaceHolder.getSurface());
            }
    ...
    public void onCameraOpened(CameraDevice cameraDevice, String cameraId, final Size previewSize, final int displayOrientation, boolean isMirror) {
        ...
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                TextView tvPreview = new TextView(MyCameraActivity.this);
                TextView tvOrigin = new TextView(MyCameraActivity.this);
    ...
    public void onPreview(final byte[] y, final byte[] u, final byte[] v, final Size previewSize, final int stride) {
        imageProcessExecutor.execute(new Runnable() {
                ...
                public void run() {
                    ...
                    if (nv21 != null)
                    ncnnbodypose.setYUV(nv21, 640, 480);
